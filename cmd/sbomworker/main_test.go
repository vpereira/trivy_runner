...
func TestProcessQueueReturnsError(t *testing.T) {
	...
	// Assuming prometheusMetrics and airbrakeNotifier are properly initialized elsewhere
	...
	_, err = rdb.RPush(context.Background(), "tosbom", "registry.suse.com/bci/bci-busybox:latest|/app/images/trivy-scan-1918888852").Result()
	if err != nil {
		t.Fatal(err)
	}
...
func TestProcessQueueReturnsSuccess(t *testing.T) {
	...
	_, err = rdb.RPush(ctx, "tosbom", "registry.suse.com/bci/bci-busybox:latest|/app/images/trivy-scan-1918888852").Result()
